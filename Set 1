...
function savings(grossPay, taxRate, expenses) {
    let AfterTaxPay = grossPay - (grossPay * taxRate)
...
function materialWaste(totalMaterial, materialUnits, numJobs, jobConsumption) {
    let totalconsumedMaterial = numJobs * jobConsumption
    let remainingMaterial = totalMaterial - totalconsumedMaterial
...
function interest(principal, rate, periods) {
    ...
    let finalinvestment = principal + interest
